...
int convolution2D(int posy, int posx, const unsigned char *restrict input, char operator[][3]) {
	...
	for (j = -1; j <= 1; j++) {
		temp4 = j + 1;
                res += input[temp1 + j] * operator[0][temp4];
                res += input[temp2 + j] * operator[1][temp4];
                res += input[temp3 + j] * operator[2][temp4];
...
double sobel(unsigned char *restrict input, unsigned char *restrict output, unsigned char *restrict golden)
{
	...
	FILE *f_in, *f_out, *f_golden;
        ...
        register int temp1 , temp2, temp3, temp4, temp5, temp6, temp7 = SIZE*SIZE, temp8;
	...
	memset(&output[temp7-SIZE], 0, SIZE*sizeof(unsigned char));
        temp3 = SIZE;
	...
	fread(input, sizeof(unsigned char), temp7, f_in);
	fread(golden, sizeof(unsigned char), temp7, f_golden);
	fclose(f_in);
	fclose(f_golden);
	...
	for (i=1; i<SIZE-1; i= i + 2 ) {
                ...
                for (j=1; j<SIZE-1; j+=1) {
                        ...
                        for (k = -1; k <= 1; k++) {
                                ...
                                res1 += input[temp4 + k] * horiz_operator[0][temp8];
                                res1 += input[temp5 + k] * horiz_operator[1][temp8];
                                res1 += input[temp6 + k] * horiz_operator[2][temp8];
                                temp8 += 1;
                        }
                        ...
                        for (k = -1; k <= 1; k++) {
                                ...
                                res2 += input[temp4 + k] * vert_operator[0][temp8];
                                res2 += input[temp5 + k] * vert_operator[1][temp8];
                                res2 += input[temp6 + k] * vert_operator[2][temp8];
                                temp8 += 1;
			...
			if (res > 255)
				...
				output[temp6] = (unsigned char)res;
...
//                         }
                        ...
                        /* Apply the sobel filter and calculate the magnitude *
			 * of the derivative.								  */
			p = convolution2D(temp6, j, input, horiz_operator)*convolution2D(temp6, j, input, horiz_operator) + 
				convolution2D(temp6, j, input, vert_operator)*convolution2D(temp6, j, input, vert_operator);
			res = (int)sqrt(p);
                        temp6 = temp2 + j;
			/* If the resulting value is greater than 255, clip it *
			 * to 255.											   */
	...
	for ( j=1; j<SIZE-1; j = j + 2 ) {
			...
			PSNR += t;
                        ...
                        temp6 = temp3 + 1;
                        t = (output[temp6] - golden[temp6])*(output[temp6] - golden[temp6]);
			PSNR += t;
	...
	fwrite(output, sizeof(unsigned char), temp7, f_out);
	fclose(f_out);
