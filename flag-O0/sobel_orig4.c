...
#define OUTPUT_FILE	"output_sobel4.grey"
...
int convolution2D(int posy, int posx, const unsigned char *input, char operator[][3]) {
	...
	res = 0;
	for (i = -1; i <= 1; i++) {
		res += input[(posy + i)*SIZE + temp1] * operator[i+1][0];
                res += input[(posy + i)*SIZE + posx ] * operator[i+1][1];
                res += input[(posy + i)*SIZE + temp2] * operator[i+1][2];
	}
	return(res);
}
		...
		for (j=1; j< (SIZE/2); j+=1 ) {
			 ...
			 * to 255.											   */
				...
				output[temp1 + j] = (unsigned char)res;
                        ...
                        /* Apply the sobel filter and calculate the magnitude *
			 * of the derivative.								  */
			p = pow(convolution2D(temp3, j, input, horiz_operator), 2) + 
				pow(convolution2D(temp3, j, input, vert_operator), 2);
			res = (int)sqrt(p);
			/* If the resulting value is greater than 255, clip it *
			 * to 255.											   */
			if (res > 255)
