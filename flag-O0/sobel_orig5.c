...
#define OUTPUT_FILE	"output_sobel5.grey"
		...
		for (j=1; j< (SIZE/2); j+=1 ) {
			 ...
			 * to 255.											   */
				...
				output[temp1 + j] = (unsigned char)res;
                        ...
                        /* Apply the sobel filter and calculate the magnitude *
			 * of the derivative.								  */
			p = convolution2D(temp3, j, input, horiz_operator)*convolution2D(temp3, j, input, horiz_operator) + 
				convolution2D(temp3, j, input, vert_operator)*convolution2D(temp3, j, input, vert_operator);
			res = (int)sqrt(p);
			/* If the resulting value is greater than 255, clip it *
			 * to 255.											   */
			if (res > 255)
